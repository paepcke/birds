...
'''
Multitaper spectrogram creation. Entirely copied from
https://github.com/preraulab/multitaper_toolbox/python
Only wrapped into a class.

See README.md in this class' directory for usage.
'''
...
class MultitaperSpectrogrammer:
# MULTITAPER SPECTROGRAM #
    @classmethod
    def multitaper_spectrogram(cls, data, fs, frequency_range=None, time_bandwidth=5, num_tapers=None, window_params=None,
      ...
      __________________________________________________________________________________________________________________
        ...
        [data, fs, frequency_range, time_bandwidth, num_tapers,
         ...
         _num_windows, nfft, detrend_opt, plot_on, verbose] = cls.process_input(data, fs, frequency_range, time_bandwidth,
                                                                           num_tapers, window_params, min_nfft,
                                                                           detrend_opt, plot_on, verbose)
    
        # Set up spectrogram parameters
        [window_idxs, stimes, sfreqs, freq_inds] = cls.process_spectrogram_params(fs, nfft, frequency_range, window_start,
                                                                              winsize_samples)
        # Display spectrogram parameters
        if verbose:
            cls.display_spectrogram_props(fs, time_bandwidth, num_tapers, [winsize_samples, winstep_samples], frequency_range,
                                      detrend_opt)
        ...
        calc_mts_segment_plus_args = partial(cls.calc_mts_segment, dpss_tapers=dpss_tapers, nfft=nfft, freq_inds=freq_inds,
        ...
        if plot_on:
            ...
            librosa.display.specshow(cls.nanpow2db(mt_spectrogram), x_axis='time', y_axis='linear',
    ...
    # Process User Inputs #
    @classmethod    
    def process_input(cls, data, fs, frequency_range=None, time_bandwidth=5, num_tapers=None, window_params=None, min_nfft=0,
    ...
    @classmethod
    def process_spectrogram_params(cls, fs, nfft, frequency_range, window_start, datawin_size):
    ...
    @classmethod
    def display_spectrogram_props(cls, fs, time_bandwidth, num_tapers, data_window_params, frequency_range, detrend_opt):
    ...
    @classmethod
    def calc_mts_segment(cls, data_segment, dpss_tapers, nfft, freq_inds, detrend_opt, num_tapers, dpss_eigen, weighting, wt):
