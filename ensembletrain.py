...
def Ensemble:
    ...
    def test_output(self):
        ...
        with torch.no_grad():
            for test_step, data in enumerate(data_loader):
                #recombining step dependent on shape of outputs tensors,
                #which i'm slowly determining experimentally with print 
                #statements. will update once I'm sure, currently logging all
                test_images, labels, path = data
                outputs = []
                for model in self.trainings:
                    outputs.append(model.model(test_images))

                #dump all data to a new file
                with open('NEWEST_ENSEMBLE_OUTPUT.txt', 'w') as file:
                    writer = csv.writer(file)
                writer.writerow(["SN", "Name", "Contribution"])
                
                fileout = open("NEWEST_ENSEMBLE_OUTPUT.txt","w") 
                fileout.write(str(outputs))
                fileout.close()
